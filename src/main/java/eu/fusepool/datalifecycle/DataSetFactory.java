...
public class DataSetFactory {
    ...
    class DataSetImpl implements DataSet {
        ...
        void initialize(final String datasetName) {
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, RDF.type, Ontology.Pipe));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, RDFS.label, new PlainLiteralImpl(datasetName)));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(DlcGraphProvider.DATA_LIFECYCLE_GRAPH_REFERENCE, Ontology.pipe, dataSetUri));
            /* waht are tasks, and what are this tripples for?*/
        // create tasks
            //rdf task
            UriRef rdfTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/rdf");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, rdfTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(rdfTaskRef, RDF.type, Ontology.RdfTask));
            // digest task
            UriRef digestTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/digest");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, digestTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(digestTaskRef, RDF.type, Ontology.DigestTask));
            // enhance task
            UriRef enhanceTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/enhance");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, enhanceTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(enhanceTaskRef, RDF.type, Ontology.EnhanceTask));
            // interlink task
            UriRef interlinkTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/interlink");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, interlinkTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(interlinkTaskRef, RDF.type, Ontology.InterlinkTask));
            // smush task
            UriRef smushTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/smush");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, smushTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(smushTaskRef, RDF.type, Ontology.SmushTask));
            // publish task
            UriRef publishTaskRef = new UriRef(dataSetUri.getUnicodeString() + "/publish");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.creates, publishTaskRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(smushTaskRef, RDF.type, Ontology.PublishTask));
        ...
        //GraphNode dlcGraphNode = new GraphNode(DATA_LIFECYCLE_GRAPH_REFERENCE, getDlcGraph());
            ...
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(rdfTaskRef, Ontology.deliverable, getSourceGraphRef()));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(getSourceGraphRef(), RDF.type, Ontology.voidDataset));
            // create the graph to store text fields extract from properties in the source rdf
            tcManager.createMGraph(getDigestGraphRef());
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(enhanceTaskRef, Ontology.deliverable, getDigestGraphRef()));
            ...
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(enhanceTaskRef, Ontology.deliverable, getEnhanceGraphRef()));
            ...
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(interlinkTaskRef, Ontology.deliverable, getInterlinksGraphRef()));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(getInterlinksGraphRef(), RDF.type, Ontology.voidLinkset));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(getInterlinksGraphRef(), Ontology.voidSubjectsTarget, getSourceGraphRef()));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(getInterlinksGraphRef(), Ontology.voidLinkPredicate, OWL.sameAs));
            ...
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(smushTaskRef, Ontology.deliverable, getSmushGraphRef()));
            // create the graph to store the result of the publishing task
            tcManager.createMGraph(getPublishGraphRef());
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(publishTaskRef, Ontology.deliverable, getPublishGraphRef()));
            // set the initial dataset status as unpublished
            UriRef statusRef = new UriRef(dataSetUri.getUnicodeString() + "/Status");
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(dataSetUri, Ontology.status, statusRef));
            dlcGraphProvider.getDlcGraph().add(new TripleImpl(statusRef, RDF.type, Ontology.Unpublished));
